...
public class BiEntityConditionTagManager implements ResourceReloader, IdentifiableResourceReloadListener {
    private static final Logger LOGGER = LogUtils.getLogger();

    public final TagGroupLoader<ConditionType> tagLoader = new TagGroupLoader<>(this::get, "tags/conditions/bientity");
    public volatile Map<Identifier, Collection<ConditionType>> tags = Map.of();

    public static BiEntityConditionTagManager CONDITION_TAG_LOADER = new BiEntityConditionTagManager();

    public Optional<ConditionType> get(Identifier id) {
        return Optional.of(BiEntityConditionRegistry.get(id));
    }
    ...
    public CompletableFuture<Void> reload(Synchronizer synchronizer, ResourceManager manager, Profiler prepareProfiler, Profiler applyProfiler, Executor prepareExecutor, Executor applyExecutor) {
        ...
        return var10000.thenCompose(synchronizer::whenPrepared).thenAcceptAsync((intermediate) -> {
            ...
            map.forEach((id, conditionTypeCompletableFuture) -> conditionTypeCompletableFuture.handle((function, ex) -> {
                if (ex != null) {
                    LOGGER.error("Failed to load bientity condition {}", id, ex);
                } else {
    ...
    public Identifier getFabricId() {
        return RA_Additions.identifier("bientity_condition_tags");
    }
