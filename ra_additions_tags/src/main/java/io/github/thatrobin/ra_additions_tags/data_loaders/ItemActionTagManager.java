...
public class ItemActionTagManager implements ResourceReloader, IdentifiableResourceReloadListener {
    private static final Logger LOGGER = LogUtils.getLogger();

    public final TagGroupLoader<ActionType> tagLoader = new TagGroupLoader<>(this::get, "tags/actions/item");
    public volatile Map<Identifier, Collection<ActionType>> tags = Map.of();

    public static ItemActionTagManager ACTION_TAG_LOADER = new ItemActionTagManager();

    public Optional<ActionType> get(Identifier id) {
        return Optional.of(ItemActionRegistry.get(id));
    }
    ...
    public CompletableFuture<Void> reload(Synchronizer synchronizer, ResourceManager manager, Profiler prepareProfiler, Profiler applyProfiler, Executor prepareExecutor, Executor applyExecutor) {
        ...
        return var10000.thenCompose(synchronizer::whenPrepared).thenAcceptAsync((intermediate) -> {
            ...
            map.forEach((id, actionTypeCompletableFuture) -> actionTypeCompletableFuture.handle((function, ex) -> {
                if (ex != null) {
                    LOGGER.error("Failed to load item action {}", id, ex);
                } else {
    ...
    public Identifier getFabricId() {
        return RA_Additions.identifier("item_action_tags");
    }
