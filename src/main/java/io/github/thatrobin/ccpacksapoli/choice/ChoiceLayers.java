...
import io.github.thatrobin.ccpacksapoli.networking.CCPacksDataLoadedCallback;
...
public class ChoiceLayers extends MultiJsonDataLoader implements IdentifiableResourceReloadListener {
    ...
    protected void apply(Map<Identifier, List<JsonElement>> loader, ResourceManager manager, Profiler profiler) {
        ...
        loader.forEach((id, jel) -> {
            ...
            jel.forEach(je -> {
                try {
                    CCPacksApoli.LOGGER.info("Trying to read layer file: " + id);
                    ...
                    CCPacksApoli.LOGGER.error("There was a problem reading Choice layer file " + id.toString() + " (skipping): " + e.getMessage());
                }
            });
        });
        // Merge phase
        ...
        CCPacksApoli.LOGGER.info("Finished loading choice layers from data files. Read " + layers.size() + " layers.");
    }
