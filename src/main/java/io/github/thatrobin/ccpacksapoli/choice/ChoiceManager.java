    ...
    protected void apply(Map<Identifier, List<JsonElement>> loader, ResourceManager manager, Profiler profiler) {
        ...
        loader.forEach((id, jel) -> jel.forEach(je -> {
            try {
                ...
                CCPacksApoli.LOGGER.error("There was a problem reading Choice file " + id.toString() + " (skipping): " + e.getMessage());
            }
        }));
        CCPacksApoli.LOGGER.info("Finished loading choices from data files. Registry contains " + ChoiceRegistry.size() + " choices.");
    }
