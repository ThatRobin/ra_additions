...
public class CCPacksModPacketC2S {
            ...
            if(!component.hasAllChoices() && !component.hasChoice(layer)) {
                ...
                if(id != null) {
                    ...
                    if(layer.contains(choice)) {
                        ...
                        component.sync();
                        CCPacksApoli.LOGGER.info("Player " + playerEntity.getDisplayName().asString() + " selected Choice: " + id + ", for layer: " + layerId);
                    } else {
                        CCPacksApoli.LOGGER.info("Player " + playerEntity.getDisplayName().asString() + " tried to select unchoosable Choice for layer " + layerId + ": " + id + ".");
                        component.setChoice(layer, Choice.EMPTY);
                    }
                    confirmChoice(playerEntity, layer, component.getChoice(layer));
                    component.sync();
                } else {
                    CCPacksApoli.LOGGER.warn("Player " + playerEntity.getDisplayName().asString() + " selected unknown Choice: " + id);
                }
            } else {
                component.sync();
                CCPacksApoli.LOGGER.warn("Player " + playerEntity.getDisplayName().asString() + " tried to select Choice for layer " + layerId + " while having one already.");
            }
        });
    }
    ...
    private static void handleHandshakeReply(MinecraftServer minecraftServer, ServerLoginNetworkHandler serverLoginNetworkHandler, boolean understood, PacketByteBuf packetByteBuf, ServerLoginNetworking.LoginSynchronizer loginSynchronizer, PacketSender packetSender) {
        if (understood) {
            ...
            boolean mismatch = clientSemVerLength != CCPacksApoli.SEMVER.length;
            for(int i = 0; i < clientSemVerLength; i++) {
                clientSemVer[i] = packetByteBuf.readInt();
                if(i < clientSemVerLength - 1 && clientSemVer[i] != CCPacksApoli.SEMVER[i]) {
                    mismatch = true;
                }
            }
            if(mismatch) {
                ...
                serverLoginNetworkHandler.disconnect(new TranslatableText("choices.gui.version_mismatch", CCPacksApoli.VERSION, clientVersionString));
            }
        } else {
            serverLoginNetworkHandler.disconnect(new LiteralText("This server requires you to install CCPacks (v" + CCPacksApoli.VERSION + ") to play."));
        }
    }
