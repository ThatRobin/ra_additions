...
import io.github.thatrobin.ccpacksapoli.power.PowerIconManager;
...
public class ChoiceDisplayScreen extends Screen {
    ...
    private void renderChoiceWindow(MatrixStack matrices, int mouseX, int mouseY) {
        ...
        if(choice != null) {
            ...
            RenderSystem.setShaderTexture(0, WINDOW);
            Text title = new TranslatableText("assets/ccpacks" + ".gui.choice.title", new TranslatableText(layer.getTranslationKey()));
    ...
    private void renderPowers() {
        ...
        for(int i = 0; i < powerTypes.size()+1; i++) {
            ...
            int height = 24;
            if(i == 0){
                buttons.add(new SelectableButtonWidget(getCurrentChoice(), getCurrentChoice().getIcon(), this.itemRenderer, x, y, width, height, getCurrentChoice().getName(),0,0,24,CCPacksApoli.identifier("textures/gui/widgets.png"),256,256, button -> {
                    description = getCurrentChoice().getDescription();
                    name = new LiteralText("Description").formatted(Formatting.UNDERLINE);
                }));
            } else {
                buttons.add(new SelectableButtonWidget(getCurrentChoice(), PowerIconManager.getIcon(powerTypes.get(i-1).getIdentifier()), this.itemRenderer, x, y, width, height, powerTypes.get(i-1).getName(), 0, 0, 24, CCPacksApoli.identifier("textures/gui/widgets.png"), 256, 256, button -> {
