...
import io.github.thatrobin.ra_additions.util.ConditionType;
import io.github.thatrobin.ra_additions.util.ItemConditionRegistry;
import io.github.thatrobin.ra_additions.util.ItemConditionTagManager;
    ...
    public static void register() {
        register(new ConditionFactory<>(RA_Additions.identifier("has_item_in_recipe"), new SerializableDataExt()
                ...
                (data, stack) -> {
                    ...
                    if(condition != null) {
                        try {
                            Recipe<?>[] recipes = RecipeFinder.getRecipesFor(stack, Apoli.server.getRecipeManager(), condition, recipeType);
                            if(recipes.length > 0) {
                                found = true;
                            }
